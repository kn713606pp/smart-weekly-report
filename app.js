...
function setupEventListeners() {
    ...
    detailModalCloseBtn.addEventListener('click', () => closeModal(taskDetailModal));
...
async function handleAddComment(e){e.preventDefault();const t=e.target.dataset.taskId,o=commentInput.value.trim();if(!o||!t)return;try{const e=collection(db,`tasks/${t}/comments`);await addDoc(e,{text:o,authorId:currentUser.uid,authorName:currentUser.displayName,createdAt:serverTimestamp()});commentInput.value=""}catch(e){console.error("新增評論失敗:",e)}}
...
function generateReport(){if(!currentUser)return;const e=new Date;e.setDate(e.getDate()-7);const t=query(collection(db,"tasks"),where("assigneeId","==",currentUser.uid),where("status","==","done"),where("completedAt",">=",e)),o=onSnapshot(t,t=>{let n=`本週 (${e.toLocaleDateString()} - ${new Date().toLocaleDateDateString()}) 已完成任務：\n\n`;t.empty?n+="- 無":t.forEach((e,t)=>{n+=`${t+1}. ${e.data().title}\n`});document.getElementById("report-content").value=n;document.getElementById("report-output").classList.remove("hidden");o()})}
